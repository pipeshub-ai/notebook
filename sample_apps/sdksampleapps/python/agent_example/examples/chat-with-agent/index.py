...
This example provides an interactive REPL to chat with a PipesHub Agent.
It creates a new agent for the current session (no reuse of existing
agents), then runs a read-eval loop: the user types a message, the script
POSTs to create or add to the conversation, and prints the agent's reply.
If the response is asynchronous, the script polls until the reply is
available or the conversation status is COMPLETED or FAILED. Type
'quit' or 'exit' to end the session.

Authentication uses the same approach as the auth example (static Bearer
token or OAuth with PKCE). The flow is:

1. Create a new agent (same as the create-agent example).
2. First user message: POST .../agents/{agent_key}/conversations (start conversation).
3. Later messages: POST .../agents/{agent_key}/conversations/{id}/messages.
4. If the reply is not in the response (async): poll GET .../conversations/{id}
   until bot content or status COMPLETED/FAILED.
...
# Display name of the agent created for this chat session (new agent per run).
...
# -------------------------------------------------------------------------
# Create Agent + Conversation API + REPL
# -------------------------------------------------------------------------
# This example: (1) Create a new agent (same as create-agent example).
# (2) First user message starts a conversation via POST .../conversations.
# (3) Later messages POST to .../conversations/{id}/messages.
# (4) If the reply is not in the response (async), poll GET .../conversations/{id}
#     until we have bot content or status COMPLETED/FAILED.
# -------------------------------------------------------------------------
...
def get_pipeshub_client(server_url: str, token: str) -> Any:
    ...
    Used here for model discovery when creating the agent. The actual
    agent creation and conversation API calls are made via REST.
...
def pick_llm_and_reasoning(
    ...
    """
    Build the models payload for POST /agents/create (prefer GPT-4o).

    Same logic as the create-agent example: selects one LLM and optionally
    a reasoning model, with REST fallback when the SDK returns no models.
...
def create_agent_for_chat(pipeshub: Any, server_url: str, token: str) -> str:
    """
    Create a new agent for this chat session via create_agent_via_api (src.index).

    Picks LLM and reasoning models, then calls the shared create_agent_via_api()
    to perform POST /agents/create. Does not reuse existing agents by name.
    ...
    # Shared helper in src.index: performs POST /agents/create.
...
def create_agent_conversation_via_api(
    ...
    agent_key : str
        Agent identifier (key returned from create agent).
...
def add_agent_message_via_api(
    ...
    with the user query. Returns the updated conversation when available.
    ...
    conversation_id : str
        Conversation identifier from create_agent_conversation_via_api or
        a previous add_agent_message_via_api response.
...
def extract_bot_content_from_dict(data: Optional[dict]) -> Optional[str]:
    ...
    if not isinstance(messages, list):
        return None
    # Scan newest-first so we return the latest bot reply.
    for m in reversed(messages):
        ...
        if msg_type == "user_query":
            continue
        # Prefer messages explicitly marked as bot/assistant.
        role = (m.get("role") or "").lower()
        if msg_type in ("bot_response", "botresponse") or role in ("assistant", "ai", "bot"):
            content = m.get("content")
            if content:
                return str(content)
        # Fallback: any message with content that is not user_query.
        content = m.get("content")
        if content:
            return str(content)
    return None
...
def run_repl(pipeshub: Any, agent_key: str, server_url: str, token: str) -> None:
    ...
    ----------
    pipeshub : Any
        Unused; kept for signature compatibility with src.index.run_repl.

    agent_key : str
        Agent to chat with (key from create_agent_for_chat).
    ...
    while True:
        ...
        try:
            # First message starts a new conversation; later messages add to it.
            if conversation_id is None:
                ...
                # Response may include updated conversation id; keep our reference in sync.
                if res_dict:
                    conversation_id = (
            ...
            # Async agent: reply may not be in POST response; poll GET until we have content or COMPLETED/FAILED.
...
def main() -> None:
    """
    CLI entrypoint: authenticate, create one agent, run chat REPL until quit/exit.

    The script resolves a Bearer token, creates a new agent for the session,
    then runs an interactive loop reading user input and printing the agent's
    replies. The REPL blocks until the user types 'quit' or 'exit'.
    """
